...
# /fix — Architect-First Fix Cycle (COM-386)
...
```
USAGE:
  /fix <description>                 Proportionate shape (default)
  /fix --narrow|--broad <description>  Bounded repair shape
  /fix --quick <description>         Quick ticket-envelope repair
  /fix --diagnose-only <description> Diagnose only; no repair contract
...
- `/test uat` is required only for a real journey or `test_uat`/`both` closure mode.
...
| /test uat: Prepare → Execute |<---- shared by /test, /fix, /jClose
...
## Typed factual intake (what `/fix` passes to jArchitect)
...
Before jArchitect launches, `/fix` presents the acceptance boundary and asks the
...
For qualifying work, `/fix` invokes the sole executable Packet v1 authority,
...
For a **ticketed qualifying broad** `/fix`, pre-implementation contract challenge
...
`/fix` validates its own invocation before any launch:
...
For qualifying work, use the current producer-readiness and Gate A receipt bindings recorded in the active Packet v1 authority. The active `forward-intelligence-activation.v1.json` authorizes qualifying dispatch after the deterministic activation check returns allowed. Ordinary non-qualifying local fixes retain their existing `/fix` path.
...
1. `/fix` consumes the `/jDebug` diagnosis-conviction receipt without reinterpretation — conviction precedes implementation.
...
9. When UAT is required, closure runs through
   **/test uat prepare -> execute -> feedback**; `/fix` cannot close acceptance.
...
UAT-required cycle returns to `/test uat prepare → execute → feedback`; `/fix cannot close acceptance`.
...
`/fix` assembles acceptance context, user authority, trusted cycle identity, policy metadata, and ground conditions, then invokes the generic jArchitect contract; it must not reteach the full architecture method. Terminal-
...
jArchitect body and its regenerated projections, not in this skill. `/fix` owns
the workflow: Choice A/B labels, the exact fix-contract filename, UAT and
`/test` routing, reviewer authorization, packet fields, path/digest receipts,
and ticket closure.
...
UAT appears only for a real user journey and runs through `/test`-owned
...
- `/fix` → `/jDebug` when diagnosis conviction is missing; the cycle consumes
  the conviction receipt.
- `/fix` → `/jPlan` when the fix reveals need for a new ticket.
- `/fix` → `/jGo` when multiple related fixes are needed.
- `/fix` → `/jClose` when the fix completes a ticket.
- `/fix` → `/test` when a real user journey requires UAT; closure runs through
  `/test uat prepare -> execute -> feedback`.
