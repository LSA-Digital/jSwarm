...
5. If failed test/run evidence says `gate`, `invariant`, `rejected`, `escalated`, `blocked`, `violation`, or `pre_conditions_failed`, route to `/jDebug`'s **rules-first discriminator** before generic tracing. A non-rule provider/runtime `429` routes to jDebug's provider branch instead.
6. After two repeated attempts without root cause, stop and use `/fix` or a debugger/architect consultation.
