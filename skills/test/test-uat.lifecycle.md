...
chain verification used by `/test uat prepare`. A successful pre-walk permits a
later prepare invocation to issue an already-populated feedback file for the
owner. The owner records observations in that file and runs `/test uat feedback`.
...
| --- | --- | --- | --- |
| `uat-scenarios.md` | Official scenario/GWT source of truth for the ticket | Process (chain binds `gwt_sha256`) + human | You + agent via `/uat author` |
| `uat-scenario-steps.md` | Delivery-journey runbook per scenario — the modern name for legacy `uat-test.md` | Human + process (jQATester pre-walk source) | You + agent via `/uat author` |
| `uat-test.md` | Legacy alias of `uat-scenario-steps.md`, resolved by the chain when the canonical file is absent; at issuance PREP gates it as the owner walk script | Human (owner walk) + tooling (chain-verifier fallback; A4 walk-script gate) | You + agent via `/uat author` |
| `<TICKET>.UAT-CURRENT-ROUND.md` | Sealed round package (`DRAFT_SEALED` → `ISSUED`) with gate cards and bound identities | Process + tooling (issuance gates, exact-file walk) | `jswarm/uat_round_materialize.py` via `/test uat prepare` |
...
You and the agent follow the canonical DevOps spine:
 ...
 [jPlan] ----------> [jGo] ----------> [jPrecompact] ---> [jClose] ---> [jMerge]
    ...
    |                        PROCEED | DISPATCH /test uat prepare <TICKET>
    |                        BLOCK   | SKIP
...
dispatches `/test uat prepare <TICKET>` when current proof is absent or stale
...
```text
                   ...
                   |                                           |
                   v                                           v
      /uat author creates current scenario/GWT          query_uat_scenarios.py
      + derived uat-test script                          --verify-chain checks
                   |                                    current scenario/GWT binding
                   v                                           |
      jQATester pre-walks exact sealed                  /test uat prepare verifies
...
| --- | --- | --- | --- | --- |
| Author scenarios | You + agent | Acceptance criteria and product intent | Official scenario/GWT and derived `uat-test` | `/uat author`; `jswarm/uat-scenarios/` |
...
| Seal package | Agent + deterministic tool | Current scenarios, derived script, verified chain, certified build | `DRAFT_SEALED` `<TICKET>.UAT-CURRENT-ROUND.md` | `/test uat prepare`; `jswarm/uat_prepare.py` |
...
| Consume and issue | Agent | Complete identity-bound PASS report | `QA_VERIFIED`, then `ISSUED`, plus feedback file | Later `/test uat prepare`; `jswarm/uat_prepare.py:342-349` |
| Owner walk | Owner | Issued round and ready feedback file | Journey verdicts and observations | `<TICKET>.uat-feedback.md` |
| Ingest feedback | Agent + deterministic tool | Newest unprocessed valid feedback file | Idempotent traceability-ledger upsert and processing receipt | `/test uat feedback`; `uat/feedback.md`, `jswarm/uat_feedback.py` |
...
When you run `/test uat feedback`, the procedure selects the **newest
...
finding or corrupt the ledger. Owner feedback that carries defects routes to a /fix cycle.
...
- [NFR verification support](nfr/verification.md) — supporting contract only;
  no active `/test nfr verify` route.
