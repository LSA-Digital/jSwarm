...
`/jTest` consumes verification evidence read-only. It does not author NFRs, mutate the NFR catalog, author thresholds, or run producers. `nfr-catalog-system` retains NFR authoring authority under the unchanged F-52 `/jconfig Installer & Component Catalog` identity.
