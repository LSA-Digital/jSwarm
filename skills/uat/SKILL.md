...
---
<!-- CONFIG-CONTROLLED (COM-176 controlled-config) — master: skills/uat/SKILL.md
...
# /uat — UAT scenario utilities menu

`/uat` is a small, extensible menu of UAT-scenario engine utilities. It is a **thin menu**: each option delegates to an associated skill that does the work.

## Usage

```
/uat                  # show this menu and ask which option to run
/uat 1                # run option 1 directly
/uat extract-assets   # run option 1 by name
/uat 1 --pdf <f.pdf> --scenarios <scenarios.json>   # run option 1 with args passed through
/uat 2                # run option 2 directly
/uat populate-content # run option 2 by name
/uat 2 --scenarios <scenarios.json>                 # run option 2 with args passed through
/uat 3                # run option 3 directly
/uat author-scenario  # run option 3 by name
/uat 3 --scenarios <scenarios.json>                 # run option 3 with args passed through
```
...
- jQATester browser dispatches (any mode) are governed by the `/test` skill's `uat.md` §"jQATester dispatch contract": COM-246 transport-ladder preflight (`PYTHONPATH="${JSWARM_HOME:-$HOME/dev/jswarm}" "${JSWARM_HOME:-$HOME/dev/jswarm}/.venv/bin/python" -m jswarm.qa_transport ...`) + the thirteen-field handoff template (`${JSWARM_HOME:-$HOME/dev/jswarm}/docs/templates/QA_DISPATCH_HANDOFF_TEMPLATE.md`). This menu never dispatches a browser agent without that contract.
