...
> **Source protocol:** Extracted from `/jGo`'s Auto-Context Management checkpoint protocol. Use this command whenever you want to safely compact context, whether or not `/jGo` is active.
...
**Step 2.0-hygiene: ticket-folder tidy via jOps (FULL MODE ONLY; fail-open).** Dispatch a `jOps` agent to tidy `.jswarm/plans/TICKET-XXX/` at the start of Surface 2 so the moves ride this checkpoint's commit:
