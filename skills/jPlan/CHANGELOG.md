...
| 2026-07-16 | Owner directive | Added orchestrator model and effort routing to the core plan header, Lite briefing skeleton, and Feature governance pattern, including phase-boundary reminders; field pattern proven in an adopting project. `template.CORE.md`'s source `template_version` remains `1`; consumers validate generated-template versions, so the core-change rule regenerates and bumps all generated templates. |
