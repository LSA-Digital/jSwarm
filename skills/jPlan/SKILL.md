...
All jCheckin telemetry writes go directly to the canonical machine-local
`common/logs.jCheckin/` store. Enrollment is per checkout; project paths are
read-only symlink views. Every lifecycle and hook path remains advisory:
record/warn/continue on operational faults, with typed event loss and no local
spool when common is unavailable.
...
6. **Parameter defaults (COM-128 WS4 — fail-open).** If `managed_commands.jPlan.md` declares an optional `parameters:` mapping, use those values as the **defaults** for the matching Step 1 questions (`default_depth`, `default_execution_team`, `default_review_tier`, `default_arch_tier`, `default_automated_uat`, `default_e2e_policy`, `default_test_data_strategy`). They pre-fill the questions; the developer still overrides every answer per ticket. A missing, unknown, or malformed `parameters:` block NEVER blocks — fall back to the global defaults stated in Step 1. Resolve deterministically (and fail-open) with:
...
## Optimization call-out (scaffold at birth)

<!-- joptimize:call-out -->
After the plan file is written and parent-tracker reconciliation completes (Step 5 outputs done), run the jOptimize check for the ticket: read its `Opt` cell and act by state — `🎯` or `—`: proceed; `⚙️`: run `${JSWARM_HOME:-$HOME/dev/jswarm}/.venv/bin/python -m jswarm.joptimize.cli --certify-check <TICKET> --format json` (preview) and present the result; `◇` or a brand-new row: offer the scaffold preview `${JSWARM_HOME:-$HOME/dev/jswarm}/.venv/bin/python -m jswarm.joptimize.cli --scaffold <TICKET> --format json`; missing/legacy (`🟡`/`🔴`)/ambiguous cell: surface a visible operator decision. Never apply automatically — apply requires the preview receipt plus explicit approval (`scaffold --apply --receipt <path> --confirm`) under this skill's safety contract.

## Check-in contract call-out

Ticket state for `checkin-review@1` (canonical contract: `skills/jCheckin/references/checkin-review.md`) lives in the plan — but only when the ticket is eligible: not Lite, not docs-only, not greenfield, and not a one-line plan. Call `/jCheckin plan`, which applies that eligibility rule and returns the plan block to carry; for an eligible ticket the returned block records, alongside the usual plan sections: declared stopping conditions, a pointer to the last check-in row, and the causal-surface identifier the contract defines for each accepted review result. An ineligible ticket adds zero fields for this contract. This skill does not evaluate or trigger the check-in contract — it only carries the state the contract reads and writes.
